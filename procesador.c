...
// Carga un programa desde el archivo en la memoria de instrucciones
void load_program(Processor *cpu, const char *filename) {
    ...
    while (fgets(line, sizeof(line), fp) != NULL && i < MAX_INSTRUCTIONS) {
        if (sscanf(line, "%5s %c %d", cpu->code_memory[i].inst, &cpu->code_memory[i].op1, &cpu->code_memory[i].op2) < 2) {
            break; // Si no hay suficientes datos, salir
        }
        i++;
    }
    fclose(fp);
...
void alu_and_control_unity(Processor *cpu) {
    ...
    int op2_val = (instr->op2 < MEM_SIZE) ? cpu->memory[instr->op2] : instr->op2; // Valor del segundo operando
    int result = 0;
    ...
    if (strcmp(instr->inst, "MOV") == 0) {
        ...
        if (instr->op2 < MAX_INSTRUCTIONS)
            cpu->PC = instr->op2 - 1; // -1 porque el PC se incrementa al final
        else
...
// Guarda el estado del procesador en un archivo
